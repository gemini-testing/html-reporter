...
describe('lib/adapters/config/testplane', () => {
    ...
    describe('getBrowserIds', () => {
        it('should return browsers from "projects" field', () => {
            ...
            assert.deepEqual(PlaywrightConfigAdapter.create(config).getBrowserIds(), ['yabro1', 'yabro2']);
        });

        it('should return default browser if "projects" are not specified', () => {
            assert.deepEqual(PlaywrightConfigAdapter.create({} as PwtConfig).getBrowserIds(), [DEFAULT_BROWSER_ID]);
        });
    });
