...
const {IMAGES_PATH, SNAPSHOTS_PATH, ERROR_DETAILS_PATH} = require('lib/constants');
...
describe('lib/merge-reports', () => {
    ...
    it('should resolve json paths and urls while merging reports', async () => {
        ...
        fs.readJSON.withArgs('src-report/path-1.json').resolves({jsonUrls: ['path-2.json', 'https://ci.ru/path-3.json'], dbUrls: ['sqlite-1.db']});
        fs.readJSON.withArgs('src-report/path-2.json').resolves({jsonUrls: [], dbUrls: ['sqlite-2.db']});
    ...
    it('should normalize urls while merging reports', async () => {
        ...
        fs.readJSON.withArgs('src-report/path-1.json').resolves({jsonUrls: ['https://foo.bar/path-2.json']});
    ...
    [IMAGES_PATH, SNAPSHOTS_PATH, ERROR_DETAILS_PATH].forEach(folderName => {
        describe(`copy artifacts from folder "${folderName}`, () => {
            it('should not copy artifacts if passed urls on database', async () => {
                const toolAdapter = stubToolAdapter({htmlReporter});
                const paths = ['https://ci.ru/sqlite.db'];
                const destPath = 'dest-report/path';
            ...
            it('should not copy artifacts if folder does not exist', async () => {
                const toolAdapter = stubToolAdapter({htmlReporter});
                const paths = ['src-report/sqlite.db'];
                const destPath = 'dest-report/path';

                const srcArtifactPath = path.resolve(process.cwd(), 'src-report', folderName);
