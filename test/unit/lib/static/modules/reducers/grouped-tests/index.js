        ...
        describe(`${actionName} action`, () => {
            it('should calc only available meta keys if tests are not grouped', () => {
                ...
                const state = {
                    ...defaultState,
                    tree: mkStateTree(),
                    view: mkStateView({groupTestsByKey: '', ...viewArgs})
                };
            ...
            it(`should group only selected "${SECTIONS.RESULT}" group with "${ERROR_KEY}" key`, () => {
                const viewArgs = mkViewArgs_();
                const groupTestsByKey = `${SECTIONS.RESULT}${KEY_DELIMITER}${ERROR_KEY}`;
                const state = {
                    ...defaultState,
                    tree: mkStateTree(),
                    view: mkStateView({groupTestsByKey, ...viewArgs}),
            ...
            it(`should group only selected "${SECTIONS.META}" group with "foo_bar" key`, () => {
                ...
                const groupTestsByKey = `${SECTIONS.META}${KEY_DELIMITER}${groupKey}`;
                const state = {
                    ...defaultState,
                    tree: mkStateTree(),
                    view: mkStateView({groupTestsByKey, ...viewArgs})
