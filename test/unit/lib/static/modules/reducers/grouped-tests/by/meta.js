...
describe('lib/static/modules/reducers/grouped-tests/by/meta', () => {
    ...
    describe('"groupMeta" method', () => {
        ...
        it('should stringify meta value if it is specified as object', () => {
            const group = {byKey: {}};
            const metaValue = {foo: 'bar'};
            const results = [{metaInfo: {aaa: metaValue}}];
            handleResults_(results);

            module.groupMeta({group, groupKey: 'aaa'});

            assert.calledOnceWith(addGroupItem, {group: sinon.match.object, result: results[0], value: JSON.stringify(metaValue)});
        });
