...
module.exports = class ResultsTreeBuilder {
    ...
    _addBrowser({id, parentId, name, version}, testResultId, attempt) {
        ...
        if (!browsers.byId[id]) {
            browsers.byId[id] = {id, parentId, name, resultIds: [], versions: []};
            browsers.allIds.push(id);
        }

        this._addResultIdToBrowser(id, testResultId, attempt);
        this._addBrowserVersion(id, version);
    }
    ...
    _addBrowserVersion(browserId, browserVersion) {
        if (!this._tree.browsers.byId[browserId].versions.includes(browserVersion)) {
            this._tree.browsers.byId[browserId].versions.push(browserVersion);
        }
    }
