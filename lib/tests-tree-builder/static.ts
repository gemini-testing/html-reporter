...
import {BrowserVersions, DB_COLUMN_INDEXES, DEFAULT_TITLE_DELIMITER, TestStatus} from '../constants';
...
export class StaticTestsTreeBuilder extends BaseTestsTreeBuilder {
    ...
    build(rows: RawSuitesRow[] = []): { tree: Tree; stats: FinalStats; skips: SkipItem[]; browsers: BrowserItem[] } {
        ...
        for (const row of rows) {
            ...
            const formattedResult = new SqliteTestResultAdapter(row, attempt, {titleDelimiter: DEFAULT_TITLE_DELIMITER});
