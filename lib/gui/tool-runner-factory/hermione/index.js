...
module.exports = class HermioneRunner extends BaseToolRunner {
    ...
    run(tests = []) {
        ...
        const testPaths = _([].concat(tests))
    ...
    updateReferenceImage(suites) {
        const formattedTests = _.flatMap([].concat(suites), (suite) => formatTests(suite, formatTestHandler));
...
function getTestPaths(test) {
    return formatTests(test, (browser) => _.get(browser, 'result.metaInfo.file'));
}
