...
function populateSuitesArray(attempt, node, suitePath) {
    ...
    if (!pathPart) {
        ...
        currentResult.attempt = prevResult.attempt + 1; //set the attempt number 1 more than the previous one
        browser.result = currentResult; //set the result to the latest attempt
        return;
    }
...
exports.renameDatabaseForReuse = (reportPath) => {
    if (!fs.existsSync(path.resolve(reportPath, constantFileNames.LOCAL_DATABASE_NAME))) {
        logger.warn(chalk.yellow(`No database to reuse in ${reportPath}`));
        return;
    }
    try {
        fs.renameSync(
            path.resolve(reportPath, constantFileNames.LOCAL_DATABASE_NAME),
            path.resolve(reportPath, constantFileNames.DATABASE_TO_REUSE)
        );
    } catch (e) {
...
exports.getDataFromDatabase = async (pathToDatabase, reportBuilder) => {
    const database = new sqlite3.Database(pathToDatabase);
    return new Promise((resolve, reject) => {
        database.all('SELECT * FROM suites', [], (err, rows) => {
            database.close();
            if (err) {
                return reject(new NestedError('Error while getting data from database ', err));
            }
