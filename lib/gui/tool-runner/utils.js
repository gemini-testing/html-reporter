...
const {mergeTablesQueries, selectAllQuery, compareDatabaseRowsByTimestamp} = require('../../common-utils');
const {createDatabaseUrlsFile} = require('../../server-utils');
...
function parseDatabaseRows(rows) {
    const suitesArray = [];
...
exports.mergeDatabasesForReuse = async (reportPath) => {
    ...
    await createDatabaseUrlsFile(reportPath, [constantFileNames.LOCAL_DATABASE_NAME]);
...
exports.getDataFromDatabase = (dbPath) => {
    try {
        ...
        const rows = db.prepare(selectAllQuery).raw().all();
        db.close();

        return parseDatabaseRows(rows.sort(compareDatabaseRowsByTimestamp));
    } catch (err) {
