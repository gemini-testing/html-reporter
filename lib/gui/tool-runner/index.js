...
module.exports = class ToolRunner {
    ...
    finalize() {
        return isSqlite(this.saveFormat) ? this._reportBuilder.closeDatabaseConnection() : this._reportBuilder.saveDataFileSync();
    }
    ...
    async _loadReuseData() {
        if (isSqlite(this.saveFormat)) {
            if (!fs.existsSync(path.resolve(this._reportPath, constantPaths.DATABASE_TO_REUSE))) {
                utils.logger.warn(chalk.yellow(`Nothing to reuse in ${this._reportPath}`));
                return {};
            }
            return await getDataFromDatabase(this._reportPath, this._reportBuilder);
        }
