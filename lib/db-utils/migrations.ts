...
export const getDatabaseVersion = (db: Database): number | null => {
    ...
    try {
        const [versionTableExists] = db.exec(`SELECT name FROM sqlite_master WHERE type='table' AND name=?`, [DB_VERSION_TABLE_NAME]);
        debug('versionTableExists:', versionTableExists);

        if (versionTableExists) {
            const [result] = db.exec(`SELECT ${VERSION_TABLE_COLUMNS[0].name} FROM ${DB_VERSION_TABLE_NAME} LIMIT 1`)[0].values;
            debug('versionRow: ', result);
