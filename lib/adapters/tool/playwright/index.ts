...
export class PlaywrightToolAdapter implements ToolAdapter {
    ...
    private _haveProjectsInConfig: boolean;
    ...
    constructor(opts: ToolAdapterOptionsFromCli) {
        ...
        this._haveProjectsInConfig = !_.isEmpty(this._config.original.projects);
    ...
    // TODO: should cpecify for testplane ???
    ...
    async readTests(): Promise<PlaywrightTestCollectionAdapter> {
        const stdout = await new Promise<string>((resolve, reject) => {
            ...
            const browserArgs = this._haveProjectsInConfig ? [] : ['--browser', DEFAULT_BROWSER_ID];
    ...
    async run(_testCollection: PlaywrightTestCollectionAdapter, tests: TestSpec[] = []): Promise<boolean> {
        ...
        return new Promise((resolve, reject) => {
            const args = prepareRunArgs(tests, this._configPath, this._haveProjectsInConfig);
...
function prepareRunArgs(tests: TestSpec[], configPath: string, haveProjectsInConfig: boolean): string[] {
    ...
    if (browserNames.size > 0) {
        const projectArgs = Array.from(browserNames).flatMap(broName => [haveProjectsInConfig ? '--project' : '--browser', broName]);
        args.push(...projectArgs);
    } else if (!haveProjectsInConfig) {
