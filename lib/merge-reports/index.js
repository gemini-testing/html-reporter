...
module.exports = async (pluginConfig, testplane, srcPaths, {destPath, headers}) => {
    ...
    await Promise.all([
        serverUtils.saveStaticFilesToReportDir(testplane.htmlReporter, pluginConfig, destPath),
        serverUtils.writeDatabaseUrlsFile(destPath, resolvedUrls)
    ]);

    await testplane.htmlReporter.emitAsync(testplane.htmlReporter.events.REPORT_SAVED, {reportPath: destPath});
};
