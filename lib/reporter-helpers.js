    ...
    return Promise.map(testResult.assertViewResults, (assertResult) => {
        ...
        if (testResult.isImageDiffError(assertResult)) {
            actions.push(
                ...
                saveTestDiffImage(testResult, reportPath, stateName),
...
exports.saveTestImages = (testResult, reportPath) => {
    ...
    if (testResult.hasDiff()) {
        actions.push(
            ...
            saveTestDiffImage(testResult, reportPath)
        );
    }
...
function saveTestDiffImage(testResult, reportPath, stateName) {
    const src = testResult.getDiffImg(stateName).path;
    const dest = utils.getDiffAbsolutePath(testResult, reportPath, stateName);

    return src
        ? utils.copyImageAsync(src, dest)
        : utils.saveDiff(
            testResult,
            dest
        );
