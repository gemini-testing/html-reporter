    ...
    _run(prepareData, prepareImages = _.noop) {
        if (!this._tool.isWorker()) { // TODO: check if this breaks anything
            ...
            if (this._config.format === 'new') {
                ...
                reportBuilder = ReportBuilderFactory.create(this._toolName, this._tool, this._config, sqliteAdapter);
            } else {
                reportBuilder = ReportBuilderFactory.create(this._toolName, this._tool, this._config);
            }
