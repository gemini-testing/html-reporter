...
module.exports = class PluginAdapter {
    ...
    _run(prepareData, prepareImages = _.noop) {
        if (!this._tool.isWorker()) { // TODO: check if this breaks anything
            ...
            if (this._config.format === 'new') {
                const sqliteAdapter = SqliteAdapter.create(this._config, 'sqlite.db').init();
                reportBuilderSqlite = ReportBuilderFactory.create(this._toolName, this._tool, this._config, sqliteAdapter);
            }
            const generateReport = Promise
                .all([
                    prepareData(this._tool, {reportBuilder, reportBuilderSqlite}, this._config),
                    prepareImages(this._tool, {reportBuilder, reportBuilderSqlite}, this._config)
                ])
