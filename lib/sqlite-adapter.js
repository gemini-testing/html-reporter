...
const debug = require('debug')('sqlite-adapter');
...
module.exports = class SqliteAdapter {
    ...
    init() {
        this._dbConn = new sqlite3.Database(path.join(this._pluginConfig.path, this._dbName), err => {
            if (err) {
                throw new Error('ERROR WHILE CREATING DB: ' + path.join(this._pluginConfig.path, this._dbName) + err.message);
            }
        });
    ...
    _insert(values) {
        this._dbConn.run(`INSERT INTO suites VALUES (${values.map(() => '?').join(', ')})`, values, err => {
            if (err) {
                throw new Error('ERROR WHILE INSERTING TO TABLE: ' + err.message);
            }
        });
    }
    ...
    _createTable() {
        this._dbConn.serialize(() => {
            ...
            this._dbConn.run('CREATE TABLE suites (' +
                ['suitePath TEXT',
                    'suiteName TEXT',
                    'name TEXT',
                    'suiteUrl TEXT',
