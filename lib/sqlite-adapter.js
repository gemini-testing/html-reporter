...
module.exports = class SqliteAdapter {
    ...
    _createDbConnection(savePath) {
        const dbPath = path.join(savePath, constantPaths.LOCAL_DATABASE_NAME);

        return new Promise((resolve, reject) => {
            const db = new sqlite3.Database(dbPath, err => {
                if (err) {
                    reject(new NestedError(`Error creating database at "${dbPath}"`, err));
                }
                resolve(db);
            });
        });
    ...
    _runQuery(query, values) {
        return new Promise((resolve, reject) => {
            this._dbConn.run(query, values, err => {
                if (err) {
                    reject(new NestedError(`Error while running query: "${query}"`, err));
                }
                resolve();
            });
        });
