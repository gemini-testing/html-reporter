...
const {createTableQuery, dbColumns} = require('./common-utils');
const {LOCAL_DATABASE_NAME, DATABASE_URLS_JSON_NAME} = require('./constants/file-names');
const {DB_TABLE_NAME} = require('./constants/database');
...
module.exports = class SqliteAdapter {
    ...
    constructor({reportPath, reuse = false}) {
    ...
    async init() {
        ...
        try {
            ...
            this._db.prepare(createTableQuery()).run();
        } catch (err) {
    ...
    write(testResult) {
        ...
        this._db.prepare(`INSERT INTO ${DB_TABLE_NAME} VALUES (${placeholders})`).run(...values);
    }
    ...
    _createValuesArray(testResult) {
        return dbColumns.reduce((acc, {name}) => {
