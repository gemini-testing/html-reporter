...
async function saveStaticFilesToReportDir(hermione, pluginConfig, destPath) {
    ...
    await Promise.all([
        ...
        copyToReportDir(destPath, ['index.html', 'report.min.js', 'report.min.css'], path.resolve(__dirname, './static')),
...
async function createDatabaseUrlsFile(destPath, dbLocations) {
    ...
    fs.writeFile(path.resolve(destPath, 'databaseUrls.js'), data, err => {
        if (err) {
            logError(err);
        }
    });
