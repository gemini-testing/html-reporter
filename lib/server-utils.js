...
const {formatBrowsers} = require('./common-utils');
...
async function saveStaticFilesToReportDir(hermione, pluginConfig, destPath) {
    ...
    await Promise.all([
        fs.writeFile(path.resolve(destPath, DATA_FILE_NAME), prepareCommonJSData(getConfig(hermione, pluginConfig)), 'utf8'),
...
async function appendMetaToDataFile(collection, destPath) {
    const dataContent = require(path.resolve(destPath, DATA_FILE_NAME));

    dataContent.browsers = formatBrowsers(collection);

    await fs.writeFile(path.resolve(destPath, DATA_FILE_NAME), prepareCommonJSData(dataContent), 'utf8');
}
