...
const getReferenceAbsolutePath = (testResult, reportDir, stateName) => {
    ...
    return normalizeToPosixPath(path.resolve(reportDir, referenceImagePath));
};

const getCurrentAbsolutePath = (testResult, reportDir, stateName) => {
    const currentImagePath = getCurrentPath(testResult, stateName);

    return normalizeToPosixPath(path.resolve(reportDir, currentImagePath));
};

const getDiffAbsolutePath = (testResult, reportDir, stateName) => {
    const diffImagePath = getDiffPath(testResult, stateName);

    return normalizeToPosixPath(path.resolve(reportDir, diffImagePath));
};

function normalizeToPosixPath(srcPath) {
    return srcPath.split('\\').join('/').split(path.sep).join('/');
}
...
function createPath(kind, result, stateName) {
    ...
    return normalizeToPosixPath(path.join.apply(null, components));
}
