...
const {hasImage, saveStaticFilesToReportDir, logError} = require('../server-utils');
...
module.exports = class ReportBuilderSqlite extends ReportBuilder {
    static async create(hermione, pluginConfig) {
        ...
        await Promise.all(
            [
                ...
                reportBuilder.createDatabaseUrlsFile(pluginConfig.path)
            ]
        );
    ...
    createDatabaseUrlsFile(destPath) {
        const jsonData = {filePaths: ['sqlite.db']};
        fs.writeFile(path.resolve(destPath, 'databaseUrls.json'), JSON.stringify(jsonData), err => {
            if (err) {
                logError(err);
            }
        });
