...
module.exports = class ReportBuilderSqlite extends ReportBuilder {
    ...
    async init() {
        ...
        this._hermione.on(
            this._hermione.events.AFTER_TESTS_READ,
            this._handleTestsHasRead.bind(this)
        );
    }
    ...
    async _handleTestsHasRead(collection) {
        const destPath = this._pluginConfig.path;

        try {
            await appendMetaToDataFile(collection, destPath);
        } catch (err) {
            logError(err);
        }
    }
