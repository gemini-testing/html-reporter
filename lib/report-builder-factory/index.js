...
module.exports = {
    create: (toolName, tool, pluginConfig, isSqlite = false) => {
        if (isSqlite) {
            const sqliteAdapter = SqliteAdapter.create(pluginConfig, 'sqlite.db').init();
            return ReportBuilderSqlite.create(tool, pluginConfig, adapters[toolName], sqliteAdapter);
