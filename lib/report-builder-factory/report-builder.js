...
const {logger, getPathsFor, hasImage, prepareCommonJSData, normalizeToPosixPath} = require('../server-utils');
...
module.exports = class ReportBuilder {
    ...
    _saveDataFile(saveFn) {
        return saveFn(
            normalizeToPosixPath(path.join(this._pluginConfig.path, 'data.js')),
    ...
    _copyToReportDir(files) {
        return Promise.map(files, (fileName) => {
            const from = normalizeToPosixPath(path.resolve(__dirname, '../static', fileName));
            const to = normalizeToPosixPath(path.join(this._pluginConfig.path, fileName));
