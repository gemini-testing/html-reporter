...
class Suites extends Component {
    ...
    _getSelectedViewModeInfo() {
        const {viewMode} = this.props;

        const matching = {
            [viewModes.SUCCESS]: 'be successed',
            [viewModes.FAILED]: 'be failed',
            [viewModes.RETRIES]: 'have retries',
            [viewModes.SKIPPED]: 'be skipped'
        };

        if (!matching[viewMode]) {
            return 'test status should match the selected view mode';
        }

        return `test should ${matching[viewMode]} due to selected view mode;`;
    }
    ...
    _getSelectedFiltersInfo() {
        ...
        if (viewMode !== viewModes.ALL) {
            msgs.push(this._getSelectedViewModeInfo());
        }
