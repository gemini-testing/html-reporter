        ...
        return (
            <div className="sections">
                {suiteIds.map((suiteId) => {
                    const sectionElem = <SectionCommon key={suiteId} suite={suites[suiteId]} filterByName={filterByName} filteredBrowsers={filteredBrowsers} />;

                    return lazyLoadOffset > 0
                        ? <LazilyRender key={suiteId} offset={lazyLoadOffset} content={sectionElem} />
                        : sectionElem;
                })}
            </div>
        );
