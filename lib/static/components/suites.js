...
class Suites extends Component {
    ...
    render() {
        const {suiteIds, filterByName} = this.props;

        return (
            <div className="sections">
                {suiteIds.map((suiteId) => {
                    return <SectionCommon key={suiteId} suiteId={suiteId} filterByName={filterByName}/>;
                })}
            </div>
        );
    }
...
export default connect(
    (state) => {
        const filterByName = state.view.filterByName;
        let suiteIds = state.suiteIds[state.view.viewMode];

        if (filterByName) {
            suiteIds = suiteIds.filter(id => shouldSuiteBeShown(state.suites[id], filterByName));
        }
