...
class GroupTestsSelect extends Component {
    ...
    _groupTestsByKey = (_, dom) => {
        if (dom.value !== this.props.groupTestsByKey) {
        ...
        return (
            <Fragment key={sectionName}>
                ...
                {keys.map(key => {
                    ...
                    const isActive = id === this.props.groupTestsByKey;
...
export default connect(
    (state) =>{
        const {view: {groupTestsByKey}, groupedTests} = state;
        const [selectedGroupSection, selectedGroupKey] = getParsedGroupTestsByKey(state);

        return {groupTestsByKey, groupedTests, selectedGroupSection, selectedGroupKey};
    },
