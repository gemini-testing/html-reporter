...
export default class DbSummaryKey extends Component {
    ...
    render() {
        ...
        const divClassName = classNames(
            'db-info',
            {'db-info_collapsed': this.state.collapsed}
        );
        ...
        const additionalInfo = this.state.collapsed ? null :
            <div>
                {fetchDetails.map(file =>
                    <p key={file.url}> <span className="db-info__url"> {file.url}
                    </span> responded with <span
                        className={file.response === 200 ? 'db-info__success' : 'db-info__fail'}>{file.response}</span>
                    </p>
                )}
            </div>;
