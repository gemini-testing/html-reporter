...
export default class StateError extends Component {
    ...
    _drawImage(image, actualImg) {
        ...
        return isNoRefImageError(this.props.error)
            ...
            : <Details title="Page screenshot" content={<Screenshot image={actualImg} noLazyLoad={true} />} extendClassNames="details_type_image" />;
    }
    ...
    _errorToElements(error) {
        return map(error, (value, key) => {
            ...
            return (
                <div className="details error__item" key={key}>
                    <div className="details__title">
                        {title}
                    </div>
                    {content && <ShowMore content={content} contentClassName="details__content" />}
                </div>
            );
        });
    }
