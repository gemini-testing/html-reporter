...
class State extends Component {
    ...
    constructor(props) {
        ...
        // if (props.state.hasOwnProperty('opened')) {
        if (props.state.controlledBy) {
            return;
        }

        const {state: {stateName}, toggleHandler} = props;
    ...
    _toggleState = () => {
        ...
        toggleHandler({stateName, opened: !opened, controlledBy: USER});
    }

    _getStateTitle(stateName, status) {
        const className = classNames(
            'state-title',
            {'state-title_collapsed': !this.props.state.opened},
            `state-title_${status}`
        );
    ...
    render() {
        const {status, expectedImg, actualImg, diffImg, stateName, diffBounds, opened} = this.props.state;
        ...
        if (isErroredStatus(status)) {
            ...
            elem = error
                ...
                : <StateFail expectedImg={expectedImg} actualImg={actualImg} diffImg={diffImg} diffBounds={diffBounds}/>;
        }
