...
class SectionBrowser extends Component {
    ...
    constructor(props) {
        ...
        // if (!props.browser.controlledBy && !props.browser.opened) {
        if (!props.browser.controlledBy) {
            this._toggleSection();
        }
    }

    _toggleSection = () => {
        const {suite: {suitePath}, browser: {name: browserId}, toggleSection, shouldBeOpened} = this.props;
    ...
    _onToggleSection = () => {
        ...
        toggleSection({suitePath, browserId, opened: !opened, controlledBy: USER});
    }
