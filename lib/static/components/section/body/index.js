...
class Body extends Component {
    ...
    _getErrors() {
        ...
        if (!Array.isArray(activeResult.error)) {
            return null;
        }

        return activeResult.error.map((error, idx) => (
            <div key={idx} className="error-tab">
                <div className="error-tab__item">
                    <hr/>
                    <StateError image={false} error={error}/>
                </div>
            </div>
        ));
    }
