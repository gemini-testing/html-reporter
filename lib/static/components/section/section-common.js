...
class SectionCommon extends Component {
    ...
    render() {
        ...
        const browsersToShow = browsers
            .filter(browser => shouldBrowserBeShown({browser, fullTestName, filteredBrowsers, errorGroupTests}));

        // determine section status depending on filtered browsers;
        // use original status for sections at higher hierarchy levels
        const status = browsersToShow.length > 0 ?
            determineStatus(browsersToShow.map(({result: {status}}) => status)) :
            suite.status;
        ...
        const browserTmpl = browsersToShow
            .map(browser => {
                return (
                    <SectionBrowser
                        key={browser.name}
