...
const MAX_AVAILABLE_PAGE_SIZE_OF_DB = 65536; // helps to speed up queries
...
async function mergeDatabases(dataForDbs) {
    ...
    mergedDbConnection.run(`PRAGMA page_size = ${MAX_AVAILABLE_PAGE_SIZE_OF_DB}`);
    mergedDbConnection.run(createTableQuery());

    for (const connection of connections) {
        mergedDbConnection.run(`ATTACH ${connection.filename} as attached`);
        mergedDbConnection.run('INSERT OR IGNORE INTO suites SELECT * FROM attached.suites');
        mergedDbConnection.run('DETACH attached');
        connection.close();
    }
