...
import {assign, clone, cloneDeep, filter, find, last, map, merge, reduce, isEmpty, flatten, uniq} from 'lodash';
import {
    dateToLocaleString,
    findNode,
    isSuiteFailed,
...
function getInitialState(data) {
    ...
    let formattedSuites = {};
    // if saveFormat == sqlite then suites will be defined only at gui mode
    if (suites) {
...
function formatBrowsersDataFromDb(browsers = []) {
    const list = flatten(browsers.values);
