...
function getChildSuitesStatus(state, suite, filteredBrowsers) {
    let childStatuses = [];

    if (suite.suiteIds) {
        childStatuses = suite.suiteIds.map((id) => state.tree.suites.byId[id].status);
    }
    ...
    if (suite.browserIds) {
        ...
        const suiteBrowserStatuses = suiteBrowsers.map(({resultIds}) => state.tree.results.byId[_.last(resultIds)].status);
