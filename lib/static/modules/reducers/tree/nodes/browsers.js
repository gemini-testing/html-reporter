...
export function calcBrowsersShowness({tree, view, app = {}, browserIds = [], diff = tree}) {
    ...
    const founded = search(nameFilter, useMatchCaseFilter);
    ...
    browserIds.forEach((browserId, index) => {
        ...
        diff.browsers.stateById[browserId] = {
            shouldBeOpened: tree.browsers.stateById[browserId]?.shouldBeOpened,
            retryIndex: tree.browsers.stateById[browserId]?.retryIndex,
            shouldBeShown,
