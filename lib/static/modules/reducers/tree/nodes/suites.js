...
export function calcAffectedSuitesShowness({tree, suiteIds, diff = tree}) {
    suiteIds
        .map(suiteId => tree.suites.byId[suiteId])
        .filter(Boolean)
        .sort((a, b) => b.suitePath.length - a.suitePath.length)
        .forEach((suite) => {
            changeSuiteState(tree, suite.id, {
                shouldBeShown: shouldSuiteBeShown(suite, tree, diff),
                checkStatus: shouldSuiteBeChecked(suite, tree, diff)
            }, diff);
        });
}
