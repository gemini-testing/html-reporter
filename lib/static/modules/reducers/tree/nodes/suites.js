...
export function calcSuitesShowness(tree, suiteIds) {
    ...
    const changeParentSuiteCb = (parentSuite) => {
        const shouldBeShown = parentSuite.suiteIds
            .some((suiteId) => tree.suites.stateById[suiteId].shouldBeShown);

        changeSuiteState(tree, parentSuite.id, {shouldBeShown});
    };
...
export function calcSuitesOpenness(tree, expand, suiteIds) {
    ...
    const changeParentSuiteCb = (parentSuite) => {
        const shouldBeOpened = parentSuite.suiteIds
            .some((suiteId) => tree.suites.stateById[suiteId].shouldBeOpened);

        changeSuiteState(tree, parentSuite.id, {shouldBeOpened});
    };
...
function getYoungestSuites(tree) {
    return tree.suites.allIds
        .filter((suiteId) => !_.isEmpty(tree.suites.byId[suiteId].browserIds))
