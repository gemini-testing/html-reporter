    ...
    switch (action.type) {
        ...
        case actionNames.INIT_STATIC_REPORT: {
            ...
            if (action.payload.preserveUiState) {
                // Preserve only user selections; derive statuses and visibility from the fresh tree.
                for (const kind of ['suites', 'browsers', 'results', 'images']) {
                    for (const id of tree[kind].allIds) {
                        const previous = state.tree[kind].stateById[id];
                        if (!previous) {
                            continue;
                        }
                        for (const key of ['shouldBeOpened', 'checkStatus']) {
                            if (previous[key] !== undefined) {
                                tree[kind].stateById[id][key] = previous[key];
                            }
                        }
                    }
                }
                updateParentsChecked(tree, [...new Set(tree.browsers.allIds.map(id => tree.browsers.byId[id].parentId))]);
            }
