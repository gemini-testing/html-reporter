import {findLast, isEmpty, get, isEqual} from 'lodash';
import {produce} from 'immer';
import actionNames from '../../action-names';
import {
    initSuitesState, changeAllSuitesState, changeSuiteState, updateSuitesStatus, getFailedRootSuiteIds,
    updateAllSuitesStatus, updateParentSuitesStatus, calcSuitesShowness, calcAffectedSuitesShowness,
...
import {initSearch, patchSearch} from '@/static/modules/search';
...
export default ((state, action) => {
    ...
    switch (action.type) {
        ...
        case actionNames.INIT_STATIC_REPORT: {
            ...
            initNodesStates({tree, view: state.view, app: state.app});
            ...
            if (staticImageAccepter.checkIsEnabled(state.config?.staticImageAccepter, state.gui)) {
                ...
                updateImagesStatus({tree, view: state.view, app: state.app, imageIdsArray, newStatus: COMMITED});
            }
        ...
        case actionNames.PATCH_TESTS_TREE: {
            ...
            const logStage = (operation, startedAt) => {
                console.info(`[watch-perf][client][#${performanceId}][reducer] ${operation}: ${(performance.now() - startedAt).toFixed(1)}ms`);
            };
            const nextState = produce(state, (draft) => {
                const {tree, view, app} = draft;
                const patch = action.payload;

                let stageStartedAt = performance.now();
                applyTreePatch(tree, patch);
                tree.lastPatch = patch;
                logStage('apply normalized tree patch', stageStartedAt);

                stageStartedAt = performance.now();
                patchSearch(tree, patch, action.payload.performance?.id);
                logStage('patch search index', stageStartedAt);

                stageStartedAt = performance.now();
                patch.results.addedIds.forEach((resultId) => {
                    changeResultState({tree, resultId, state: {matchedSelectedGroup: false}});
                });
                if (patch.images.addedIds.length) {
                    calcImagesOpenness({tree, expand: view.expand, imageIds: patch.images.addedIds});
                }
                logStage('initialize result/image states', stageStartedAt);

                stageStartedAt = performance.now();
                const affectedBrowserIds = Object.keys(patch.browsers.byId);
                affectedBrowserIds.forEach((browserId) => {
                    if (patch.browsers.addedIds.includes(browserId)) {
                        changeBrowserState(tree, browserId, {checkStatus: UNCHECKED});
                    }
                    setBrowsersLastRetry(tree, browserId);
                });

                if (affectedBrowserIds.length) {
                    if (view.keyToGroupTestsBy) {
                        affectedBrowserIds.forEach(browserId => changeBrowserState(tree, browserId, {shouldBeShown: false}));
                    } else {
                        calcBrowsersShowness({tree, view, app, browserIds: affectedBrowserIds});
                    }
                    calcBrowsersOpenness({tree, expand: view.expand, browserIds: affectedBrowserIds});
                }
                logStage('update browser states', stageStartedAt);

                stageStartedAt = performance.now();
                patch.suites.addedIds.forEach((suiteId) => {
                    changeSuiteState(tree, suiteId, {checkStatus: UNCHECKED});
                });

                const existingAffectedSuiteIds = patch.affectedSuiteIds.filter(suiteId => tree.suites.byId[suiteId]);
                const deepestAffectedSuiteIds = existingAffectedSuiteIds
                    .filter(suiteId => !tree.suites.byId[suiteId].suiteIds?.some(childId => existingAffectedSuiteIds.includes(childId)));
                updateParentSuitesStatus(tree, deepestAffectedSuiteIds, app.filteredBrowsers);
                if (view.keyToGroupTestsBy) {
                    patch.suites.addedIds.forEach(suiteId => changeSuiteState(tree, suiteId, {shouldBeShown: false}));
                } else {
                    calcAffectedSuitesShowness({tree, suiteIds: existingAffectedSuiteIds});
                }
                const affectedSuiteIds = Object.keys(patch.suites.byId);
                if (affectedSuiteIds.length) {
                    calcSuitesOpenness({tree, expand: view.expand, suiteIds: affectedSuiteIds});
                }
                tree.suites.failedRootIds = getFailedRootSuiteIds(tree.suites);
                logStage('update suite statuses/states', stageStartedAt);
            });

            logStage('Immer finalize and reducer total', reducerStartedAt);
        ...
        case actionNames.SELECT_ALL: {
            changeVisibleNodesCheckStatus(state.tree, {checkStatus: CHECKED}, diff.tree);

            return applyStateUpdate(state, diff);
        }

        case actionNames.DESELECT_ALL: {
            changeVisibleNodesCheckStatus(state.tree, {checkStatus: UNCHECKED}, diff.tree);
        ...
        case actionNames.TOGGLE_TESTS_GROUP: {
            ...
            calcBrowsersShowness({tree, view, app, browserIds, diff: diff.tree});
        ...
        case actionNames.STATIC_ACCEPTER_STAGE_SCREENSHOT: {
            const {tree, view, staticImageAccepter, app} = state;
            ...
            updateImagesStatus({tree, view, app, images, diff: diff.tree});
        ...
        case actionNames.STATIC_ACCEPTER_UNSTAGE_SCREENSHOT: {
            ...
            calcBrowsersShowness({tree, view, app: state.app, browserIds: failedBrowserIds, diff: diff.tree});
    ...
    return produce(state, (draft) => {
        switch (action.type) {
            ...
            case actionNames.COMMIT_REVERTED_IMAGES_TO_TREE: {
                ...
                calcBrowsersShowness({tree, view, app});
...
function initNodesStates(state) {
    ...
    initBrowsersState(tree, view, app);
    initSuitesState(tree, view);
    initImagesState(tree, view.expand);
    initResultsState(tree);
}

function applyTreePatch(tree, patch) {
    patch.suites.removedIds.forEach((suiteId) => {
        const suite = tree.suites.byId[suiteId];
        if (suite) {
            delete tree.suites.byHash[suite.hash];
        }
    });

    ['suites', 'browsers', 'results', 'images'].forEach((collectionName) => {
        const collection = tree[collectionName];
        const collectionPatch = patch[collectionName];
        const removedIds = new Set(collectionPatch.removedIds);

        collectionPatch.removedIds.forEach((id) => {
            delete collection.byId[id];
            delete collection.stateById[id];
        });
        Object.assign(collection.byId, collectionPatch.byId);
        if (collectionPatch.removedIds.length || collectionPatch.addedIds.length) {
            collection.allIds = collection.allIds.filter(id => !removedIds.has(id)).concat(collectionPatch.addedIds);
        }
    });

    Object.values(patch.suites.byId).forEach((suite) => {
        tree.suites.byHash[suite.hash] = suite;
    });
    if (!isEqual(tree.suites.allRootIds, patch.suites.allRootIds)) {
        tree.suites.allRootIds = patch.suites.allRootIds;
    }
}
...
function changeVisibleNodesCheckStatus(tree, state, diff = tree) {
    const affectedParentIds = new Set();

    tree.browsers.allIds.forEach((browserId) => {
        if (tree.browsers.stateById[browserId].shouldBeShown) {
            changeBrowserState(tree, browserId, state, diff);
            const parentId = tree.browsers.byId[browserId].parentId;
            if (parentId) {
    ...
    tree.suites.allIds.forEach((suiteId) => {
        if (tree.suites.stateById[suiteId].shouldBeShown) {
            changeSuiteState(tree, suiteId, state, diff);
            const parentId = tree.suites.byId[suiteId].parentId;
            if (parentId) {
                affectedParentIds.add(parentId);
            }
        }
    ...
    updateParentsChecked(tree, Array.from(affectedParentIds), diff);
