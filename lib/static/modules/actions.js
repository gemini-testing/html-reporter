...
import {assign, flatMap, filter} from 'lodash';
...
export const acceptAll = (fails, actionName = actionNames.ACCEPT_ALL) => {
    ...
    return async (dispatch) => {
        try {
            ...
            // await Promise.all(formattedFails.map((f) => {
            //     return axios.post('/update-reference', [f]);
            // }));
...
export const updateResult = (result) => {
    return ({type: actionNames.UPDATE_RESULT, payload: result});
};
