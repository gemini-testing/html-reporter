...
import {APP} from '../../constants/controllers';
...
export const acceptOpened = (fails) => {
    return async (dispatch) => {
        dispatch({type: actionNames.TOGGLE_SECTION, payload: {suitePath: ['TEST'], controlledBy: APP, opened: false}});
    };
...
export const toggleSection = (result) => ({type: actionNames.TOGGLE_SECTION, payload: result});
...
export const broadcastToNodes = (payload) => ({type: actionNames.BROADCAST_TO_NODES, payload});
...
function getOpenedBrowserResult({result, retries, retryIndex}) {
    const broResult = retries.concat(result)[retryIndex];

    return broResult.opened ? broResult : {};
}

// function getOpenedBrowserResult({result, retries, opened, retryIndex}) {
//     return opened ? retries.concat(result)[retryIndex] : {};
// }
