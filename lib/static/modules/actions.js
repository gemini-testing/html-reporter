...
import {LOCAL_DATABASE_NAME} from '../../constants/paths';
...
export const fetchDb = (databaseUrlsFile) => {
    return async (dispatch) => {
        try {
            const dbLocations = await fetchDbLocations(databaseUrlsFile);
            const stats = {
            ...
            const dbs = await Promise.all(dbLocations.map(async (url) => {
                ...
                stats.dbs.push({url, response: db.response});
                if (db.connection) {
                    stats.fetched++;
            ...
            const openedDbs = dbs.filter(connection => connection);
