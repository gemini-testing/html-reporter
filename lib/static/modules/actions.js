...
export const fetchDbLocations = async () => {
    // return new Promise((resolve) => {
    //     let xhr = new XMLHttpRequest();
    //     xhr.open('GET', 'databaseUrls.json', true);
    //     xhr.onreadystatechange = function() {
    //         if (xhr.readyState === XMLHttpRequest.DONE) {
    //             switch (xhr.status) {
    //                 case 200: {
    //                     resolve(JSON.parse(this.response).filePaths);
    //                     break;
    //                 }
    //                 case 404: {
    //                     resolve(['sqlite.db']);
    ...
    let dbs = await axios.get('databaseUrls.json');
    switch (dbs.status) {
        case 200: {
            return dbs.data.filePaths;
        }
        case 404: {
            return ['sqlite.db'];
        }
...
export const fetchDb = () => {
    return async (dispatch) => {
        try {
            const dbLocations = await fetchDbLocations();
            const stats = {
                expected: dbLocations.length,
                fetched: 0,
                dbs: []
            };
            const dbs = await Promise.all(dbLocations.map(url => createDb(url)));
            console.log(dbs);
            const openedDbs = [];
            ...
            for (let i = 0; i < dbs.length; i++) {
                if (dbs[i].db) {
                    stats.fetched++;
                    stats.dbs.push({url: dbLocations[i], response: dbs[i].response});
                    openedDbs.push(dbs[i].db);
                    sumOfChunkSizes += dbs[i].size;
                } else {
                    stats.dbs.push({url: dbLocations[i], response: dbs[i].response});
                }
            }

            // console.log(openedDbs);
