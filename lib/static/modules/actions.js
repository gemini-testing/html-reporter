...
export const fetchDbLocations = async (databaseUrlsFile) => {
    try {
        let dbs = await axios.get(databaseUrlsFile);
        return {
            filePaths: dbs.data.filePaths,
            response: dbs.status
        };
    } catch (e) {
        return {
...
export const fetchDb = (databaseUrlsFile) => {
    return async (dispatch) => {
        try {
            ...
            const fetchingLocationsResult = await fetchDbLocations(databaseUrlsFile);
            const dbLocations = fetchingLocationsResult.filePaths;

            stats.fetchDetails.push({url: databaseUrlsFile, response: fetchingLocationsResult.response});
            stats.expected = dbLocations.length;

            const dbs = await Promise.all(dbLocations.map(async (url) => {
                const db = await createDb(url);
                stats.fetchDetails.push({url, response: db.response});
                if (db.connection) {
                    stats.fetched++;
