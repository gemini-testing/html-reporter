...
const fetchDbLocations = () => {
    return new Promise((resolve) => {
        let xhr = new XMLHttpRequest();
        xhr.open('GET', 'dbLocations.json', true);
    ...
    return async (dispatch) => {
        try {
            ...
            for (let i = 0; i < dbLocations.length; i++) {
                const db = await createDb(dbLocations[i]);
                if (db) {
                    stats.fetched++;
                    stats.dbs.push({url: dbLocations[i], fetched: true});
                    openedDbs.push(db);
                } else {
                    stats.dbs.push({url: dbLocations[i], fetched: false});
                }
            }

            console.log(openedDbs);
            const resultDb = openedDbs.length === 1 ? openedDbs[0] : await mergeDbs(openedDbs);
