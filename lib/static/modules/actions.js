...
import {assign, filter, flatMap, set, get, cloneDeep, compact} from 'lodash';
...
function formatTests(test) {
    ...
    return flatMap(test.browsers, (browser) => {
        const browserResult = getOpenBrowserResult(browser);
...
function filterBrowsers(suites = [], filterFn) {
    const modifySuite = (suite) => {
        ...
        return set(suite, 'browsers', filter(suite.browsers, (bro) => {
            ...
            return filterFn(getOpenBrowserResult(bro));
        }));
    };
...
function getOpenBrowserResult({result, retries, state}) {
