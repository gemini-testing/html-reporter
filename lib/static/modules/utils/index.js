...
export function fuzzyMatch(text, query, matchCase = false) {
    if (!matchCase) {
...
export function isTestNameMatchFilters(
    testName, browserName, testNameFilter, strictMatchFilter, matchCase = false, useRegex = false, useFuzzy = true
) {
    if (!testNameFilter) {
        return true;
    }
    ...
    if (useRegex) {
        try {
            ...
            if (regex.test(testName) || regex.test(`${testName} ${browserName}`)) {
                return true;
            }
        } catch (e) {
            return false;
    ...
    let filterRegExpStr = escapedFilter.replace(/ /g, '(?: | › )');

    if (strictMatchFilter) {
        filterRegExpStr = `^${filterRegExpStr}$`;
    }
    ...
    try {
        ...
        // try fuzzy search or return false
    }

    if (!strictMatchFilter && useFuzzy) {
        return fuzzyMatch(testName, testNameFilter, matchCase) ||
            fuzzyMatch(`${testName} ${browserName}`, testNameFilter, matchCase);
    }
