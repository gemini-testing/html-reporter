...
export function matchTestName(
    testName, browserName, testNameFilter, {strictMatchFilter, useMatchCaseFilter = false, useRegexFilter = false} = {}
) {
    if (!testNameFilter) {
    ...
    if (testNameFilter.length < 256) {
        return fuzzyMatch([
            {title: `${testName} ${browserName}`}
        ], {$or: [{title: testNameFilter}, {title: keyboardLayoutConverter(testNameFilter)}]}, useMatchCaseFilter);
    }

    return `${testName} ${browserName}`.includes(testNameFilter);
}
