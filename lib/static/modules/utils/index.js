...
export function fuzzyMatch(variants, query, matchCase = false) {
    ...
    const bestMatch = matches.reduce((min, current) => current.score < min.score ? current : min, matches[0]);

    return {isMatch: true, score: 1 - bestMatch.score};
}
...
export function matchTestName(
    ...
    if (!isNewUi) {
        const isMatch = legacyIsTestNameMatch(testName, browserName, testNameFilter, strictMatchFilter);
    ...
    if (useRegexFilter) {
        try {
            ...
            return {isMatch: regex.test(testName) || regex.test(`${testName} ${browserName}`), score: 0};
        } catch (e) {
    ...
    if (testNameFilter.length < 256) {
        if (useMatchCaseFilter) {
            const name = `${testName} ${browserName}`;
            const match = name.includes(testNameFilter) || name.includes(keyboardLayoutConverter(testNameFilter));
            return {
                isMatch: match,
                score: 1
            };
        } else {
            const name = `${testName} ${browserName}`.toLowerCase();
            const searchName = testNameFilter.toLowerCase();

            const match = name.includes(searchName) || name.includes(keyboardLayoutConverter(searchName));
            return {
                isMatch: match,
                score: 1
            };
    ...
    return {
        isMatch: `${testName} ${browserName}`.includes(testNameFilter),
        score: 0
    };
}
