...
import {dateToLocaleString, findNode, isSuiteFailed, isDbSuiteFailed, setStatusForBranch, setStatusToAll, sortByTimeStamp, updateSuitesStats} from './utils';
...
function createTestResultsFromDb(state, action) {
    ...
    // const values = db.exec('select * from suites')[0].values.sort(sortByTimeStamp);
    // console.log(values);
    let values = db.exec('select * from suites')[0].values;
    console.log(values);
    console.time('sorting suites');
    values = values.sort(sortByTimeStamp);
    console.timeEnd('sorting suites');
