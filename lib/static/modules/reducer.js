...
function getInitialState(data) {
    ...
    const {
        ...
        failed, skipped, warned, retries, perBrowser, apiValues, gui = false, date
    } = data;
...
function createTestResultsFromDb(state, action) {
    ...
    const values = db.exec('select * from suites')[0].values;
    db.close();
    console.log(values);

    const {suites, suiteIds} = formatSuitesDataFromDb(values);
...
function insertIntoSuiteResult(acc, suite) {
    ...
    for (let i = 0; i < acc[suiteId].children.length; i++) {
        if (acc[suiteId].children[i].name === suite.children[0].name) {
            for (let k = 0; k < acc[suiteId].children[i].browsers.length; k++) {
                if (acc[suiteId].children[i].browsers[k].name === suite.children[0].browsers[0].name) {
                    acc[suiteId].children[i].browsers[k].retries.push(suite.children[0].browsers[0].result);
                    return;
                }
            }
            acc[suiteId].children[i].browsers.push(suite.children[0].browsers[0]);
            return;
        }
    }
    acc[suiteId].children.push(suite.children[0]);

    // if (acc[suiteId]) {
    //     for (let i = 0; i < acc[suiteId].children.length; i++) {
    //         if (acc[suiteId].children[i].name === suite.children[0].name) {
    //             for (let k = 0; k < acc[suiteId].children[i].browsers.length; k++) {
    //                 if (acc[suiteId].children[i].browsers[k].name === s[columns.name]) {
    //                     acc[suiteId].children[i].browsers[k].retries.push(suite.children[0].browsers[0].result);
    //                     break;
    //                 }
    //             }
...
function formatSuitesDataFromDb(suites = []) {
    return {
        suites: reduce(suites, (acc, s) => {
            ...
            insertIntoSuiteResult(acc, suite);
            // acc[suiteId] = suite;
            return acc;
        }, {}),
        suiteIds: {
