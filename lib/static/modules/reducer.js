...
import {dateToLocaleString, findNode, isSuiteFailed, isDbSuiteFailed, setStatusForBranch, setStatusToAll} from './utils';
...
function createTestResultsFromDb(state, action) {
    ...
    const values = db.exec('select * from suites order by timestamp ASC')[0].values;
    console.log(values);
    ...
    return {
        ...
        stats: {
            ...state.stats,
            all: {
                failed: suitesStats.failed,
                passed: suitesStats.passed,
                retries: suitesStats.retries,
                skipped: suitesStats.skipped,
                total: suitesStats.total
            }
        },
...
function updateSuitesStats(stats, status, suiteId) {
    switch (status) {
        case testStatus.ERROR: {
            if (stats.failedTestIds[suiteId]) {
                stats.retries++;
                return;
            }
            stats.failedTestIds[suiteId] = true;
            stats.failed++;
            stats.total++;
            return;
        }
        case testStatus.SUCCESS: {
            if (stats.passedTestIds[suiteId]) {
                stats.retries++;
                return;
            }
            if (stats.failedTestIds[suiteId]) {
                delete stats.failedTestIds[suiteId];
                stats.failed--;
                stats.passed++;
                stats.retries++;
                return;
            }
            stats.passedTestIds[suiteId] = true;
            stats.passed++;
            stats.total++;
            return;
        }
        case testStatus.SKIPPED: {
            ...
            if (stats.failedTestIds[suiteId]) {
                delete stats.failedTestIds[suiteId];
                stats.failed--;
                updateSuitesStats(stats, testStatus.SUCCESS, suiteId);
                return;
            }
            stats.total++;
        }
...
function findPlaceToInsert(suite, node, suitePath, suitesStats) {
    ...
    if (pathPart === undefined) {
        ...
        let suiteId = suite.suitePath.join('') + suite.children[0].browsers[0].name;
...
function formatSuitesDataFromDb(suites = []) {
    ...
    return {
        suites: reduce(suites, (acc, s) => {
                    ...
                    browsers: [
                        {
                            ...
                            result: {
                                ...
                                skipReason: s[columns.skipReason]
                            },
                            retries: []
                        }
                    ]
            ...
            suitePath.shift();
            findPlaceToInsert(suite, acc[suiteId], suitePath, suitesStats);
            return acc;
        }, {}),
        suiteIds: {
...
function getSuiteIdsFromDb(suites = []) {
    return [...new Set(map(suites, getSuiteIdFromDb))].sort();
}
