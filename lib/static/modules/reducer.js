...
function findPlaceToInsert(attempt, node, suitePath, suitesStats) {
    const pathPart = suitePath.shift();
    if (!pathPart) {
        node.browsers = Array.isArray(node.browsers) ? node.browsers : [];
        const attemptId = attempt.suitePath.join('') + attempt.children[0].browsers[0].name;
        const browserResult = attempt.children[0].browsers[0];
        // switch (suite.status) {
        //     case testStatus.ERROR: {
        //         setErrorToParents(node, attemptId);
        //         break;
        //     }
        //     case testStatus.SUCCESS: {
        //         removeErrorFromParents(node, attemptId);
        //         break;
        //     }
        ...
        updateSuitesStats(suitesStats, attempt.status, attemptId);
        console.log(suitesStats);
    ...
    findPlaceToInsert(attempt, child, suitePath, suitesStats);
}
...
function formatSuitesDataFromDb(rows = []) {
    ...
    for (const attempt of rows) {
        const suiteId = getSuiteIdFromDb(attempt);
        ...
        const [_, ...suitePath] = formattedAttempt.suitePath;
        findPlaceToInsert(formattedAttempt, suitesTree[suiteId], suitePath, suitesStats);
        setStatusForBranch(suitesTree, formattedAttempt.suitePath);
    }
    return {
        suites: suitesTree,
        suiteIds: {
            all: getSuiteIdsFromDb(rows).sort(),
            failed: getFailedSuiteIdsFromDb(rows)
        },
        suitesStats
    };
}
