...
import {assign, merge, filter, map, clone, cloneDeep, reduce, find} from 'lodash';
...
export default function reducer(state = getInitialState(compiledData), action) {
    switch (action.type) {
        ...
        case actionNames.TOGGLE_SECTION: {
            const {browserId, opened} = action.payload;
            const suitePath = action.payload.suitePath.slice();

            return update(state, {
                suites: {
                    [suitePath.shift()]: {
                        $updateSuite: {suitePath, browserId, value: {opened}}
                    }
                }
            });
        }
        ...
        case actionNames.CHANGE_TEST_RETRY: {
            ...
            return updateBrowser(state, action, {retryIndex});
        }
        default:
            return state;
    }
...
function updateStateResult(state, action) {
    const {browserId, stateName, retryIndex, opened} = action.payload;
    const suitePath = action.payload.suitePath.slice();
    ...
    return update(state, {
        suites: {
            [suitePath.shift()]: {
                $updateState: {
                    suitePath, browserId, stateName, retryIndex, value: {opened}
                }
            }
        }
    });
