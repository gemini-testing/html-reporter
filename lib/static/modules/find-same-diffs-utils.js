...
export const getAllOpenedImagesInfo = (fails = []) => {
    const findImagesInfo = (node) => {
        if (node.children) {
            return flatMap(node.children, findImagesInfo);
        }

        if (node.browsers) {
            return flatMap(node.browsers, (bro) => {
                if (!get(bro, 'state.opened')) {
                    return [];
                }

                return findImagesInfo({suitePath: node.suitePath, ...bro});
            });
        }
    ...
    return flatMap(fails, findImagesInfo);
};
