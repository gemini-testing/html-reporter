...
import {forOwn, pick, isArray, find, get, values} from 'lodash';
...
function filterBrowsers(suite, filteredBrowsers) {
    if (suite.hasOwnProperty('children')) {
        suite.children = suite.children.map((child) => filterBrowsers(child, filteredBrowsers));
        return suite;
    }

    suite.browsers = suite.browsers.filter((browser) => filteredBrowsers.includes(browser.name));
    return suite;
}
