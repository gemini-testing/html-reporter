...
export const getCheckedTests = createSelector(
    getBrowserIds, getBrowsers, getBrowsersStates,
    (browserIds, browsers, browsersStates) => {
        const checkedBrowsers = browserIds.filter((browserId) => browsersStates[browserId].checkStatus);

        return checkedBrowsers.map((browserId) => ({
            testName: browsers[browserId].parentId,
...
export const getAcceptableImagesByStateName = createSelector(
    ...
    (browsersStates, browsers, results, images) => {
        ...
        const acceptableImages = preparedImages.reduce((acc, image) => {
...
const getExpandedTreeNodesById = (state) => (state.ui.suitesPage.expandedTreeNodesById);
const getTreeEntityId = (result) => result.suitePath.join('/');

const getAcceptableOrRevertableImages = (
    browserIds, browsersById, resultsById, imagesById, isStaticImageAccepterEnabled, gui, expandedTreeNodesById
) => {
    ...
    return visibleResultIds.flatMap((resultId) => {
        const suitePath = getTreeEntityId(resultsById[resultId]);
        if (expandedTreeNodesById && !expandedTreeNodesById[suitePath]) {
            return [];
        }
