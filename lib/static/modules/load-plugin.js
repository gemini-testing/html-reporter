...
async function initPlugin(plugin, pluginName) {
    try {
        ...
        if (Array.isArray(plugin)) {
            ...
            // code smell around await: not cyclic dep, but the only way to redefine helpers from plugins
            const selectors = {
                helpers: await import('./selectors/helpers'),
                stats: await import('./selectors/stats'),
                tree: await import('./selectors/tree'),
                view: await import('./selectors/view')
            };
            return plugin(...depArgs, {pluginName, actions, selectors});
        }
