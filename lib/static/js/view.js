...
export default class View {
    ...
    bindShowOnlyErrors(handler) {
        const showOnlyErrorsBtn = $byId('showOnlyErrors');
        $on(showOnlyErrorsBtn, 'click', ({target}) => {
            View._toggleVisibility({target});
            const isOnlyErrorsMode = showOnlyErrorsBtn.classList.contains('button_checked');
            handler(isOnlyErrorsMode);
        });
    }
    ...
    _prepareSections() {
        this._initSectionElements();
        this._initSectionHandlers();
    }

    _initSectionElements() {
        this._rootSection = querySelector('.sections');
        this._sectionElems = querySelectorAll('.section');
    }

    _initSectionHandlers() {
